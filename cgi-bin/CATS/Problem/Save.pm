...
sub set_contest_problem_des {
    my ($des, $cid, $pid) = @_;
    my @rv = $dbh->selectrow_array(q~
        SELECT CP.id FROM contest_problems CP
        WHERE CP.contest_id = ? AND CP.problem_id = ?~, undef, $cid, $pid);
    my $cpid = $rv[0];
    _set_contest_problem_des($des, $cpid);
}

sub _set_contest_problem_des {
    my ($des, $cpid) = @_;

    my $all_des = $dbh->selectall_arrayref(q~
