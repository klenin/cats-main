...
sub _order_by_apply_dir {
    ...
    map m/[\s](DESC|ASC)$/ ? $_ : "$_ $suffix", ref $order_by ? @$order_by: $order_by;
}
...
sub order_by {
    ...
    my @order = (
        ...
        $c && ref $c->{order_by} ne 'CODE' ? _order_by_apply_dir($c->{order_by}, $s->{sort_dir}) : (),
    );
...
sub define_transforms { $_[0]->qb->define_transforms($_[1]) }
sub define_casts { $_[0]->qb->define_casts($_[1]) }
