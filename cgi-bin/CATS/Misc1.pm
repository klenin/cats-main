...
sub user_authorize {
    ...
    $uid = undef;
    $team_name = undef;
    # Á×ÔÏÒÉÚÁÃÉÑ ĞÏÌØÚÏ×ÁÔÅÌÑ É ÕÓÔÁÎÏ×ËÁ ÆÌÁÇÁ ÁÄÍÉÎÉÓÔÒÁÔÏÒÁ ÓÉÓÔÅÍÙ
    if ($sid ne '') 
    {
        my $srole;
    ...
    # ĞÏÌÕŞÅÎÉÅ ÉÎÆÏÒÍÁÃÉÉ Ï ÔÅËÕİÅÍ ÔÕÒÎÉÒÅ É ÕÓÔÁÎÏ×ËÁ ÔÕÒÎÉÒÁ ĞÏ-ÕÍÏÌŞÁÎÉÀ
    $cid = url_param('cid') || '';
    if ($cid ne '') 
    {
    ...
    {
        # ÔÒÅÎÉÒÏ×ÏŞÎÁÑ ÓÅÓÓÉÑ ĞÏ ÕÍÏÌŞÁÎÉÀ
    ...
    $virtual_diff_time = 0;
    # Á×ÔÏÒÉÚÁÃÉÑ ĞÏÌØÚÏ×ÁÔÅÌÑ × ÔÕÒÎÉÒÅ
    if (defined $uid)
    {
	...
	if (!defined $virtual_diff_time || $virtual_diff_time eq '')
	{
        ...
        if (!$is_jury)
        {
            ...
            # ÄÏ ÎÁŞÁÌÁ É ĞÏÓÌÅ ÏËÏÎŞÁÎÉÑ ÔÕÒÁ ËÏÍÁÎÄÁ ÉÍÅÅÔ ÔÏÌØËÏ ĞÒÁ×Á ÇÏÓÔÑ
            if (!$started || $finished)
            {
                $is_team = 0;
            }
