...
use fields qw(
    ...
    encoding stml zip zip_archive old_title replace remote_url tag_stack has_checker de_list run_method
);
...
sub get_log
{
    my ($pid, $sha) = get_repo_id(@_);
    return get_repo($pid, $sha, 0)->log(sha => $sha);
}
...
{
    my CATS::Problem $self = shift;
    ($self->{source}, $self->{contest_id}, $self->{id}, $self->{replace}, $self->{remote_url}, my $message, my $is_amend) = @_;
...
sub end_tag_Problem
{
    ...
    ? q~
        ...
        SET
            ...
            max_points=?, run_method=?, remote_url=?, hash=NULL
        WHERE id = ?~
    : q~
        INSERT INTO problems (
            ...
            max_points, run_method, remote_url, id
        ) VALUES (
            ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,?,?,?,?,?,?
        )~;
    ...
    $c->bind_param($i++, $self->{remote_url});
